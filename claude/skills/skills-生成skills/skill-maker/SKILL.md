...
**绝对不能把任何敏感信息存放到 skill 中。** skill 文件（SKILL.md、assets/、README.md 等）会随代码仓库一起被提交、同步、分享，一旦写入就可能进入 git 历史并被任何能看到仓库的人获取，事后删除也无法从历史中彻底清除。

- **禁止写入**：密码、API Key/Token、私钥/证书、bind 密码、数据库连接串中的口令、内网真实 IP 与主机名清单（涉密时）、真实用户名/邮箱等个人信息。
- **示例必须用占位符**：文档里演示凭据格式时，一律用 `<密码>`、`<your-api-key>`、`<bind-dn>`、`cn=manager,dc=example,dc=com` 这类明显的占位符，绝不能用真实值（哪怕"顺手"从真实环境复制过来）。
- **敏感信息应通过外部注入**：让 skill 在**运行时**从环境变量、`chmod 600` 的本地凭据文件（且加入 `.gitignore`）、或系统密钥库（`secret-tool`/`pass`/keychain）读取，而不是写死在 skill 里。
- **创建 skill 前主动排查**：若用户提供的需求或示例中夹带了真实凭据，必须提醒用户替换为占位符，并说明原因，绝不原样写入文件。
- **发现即上报**：若在既有 skill 中发现疑似真实敏感信息，应立即告知用户移除，并提醒其轮换（更换）该凭据、必要时清理 git 历史。
...
- 当skill出现问题，通过探索、用户引导等方式找到正确的方式时，最后询问用户，给出完善的建议，并优化此skill。
...
* 🔒 **绝对不能把任何敏感信息（密码、密钥、Token、私钥等）写入 skill 文件**；示例一律用占位符，敏感数据在运行时从环境变量/密钥库/`chmod 600` 且 gitignore 的文件注入（详见"安全红线")。
* Skill 名称必须与目录名一致
